...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS leads(
        ...
        submit_latitude TEXT,
        submit_longitude TEXT,
        submit_accuracy TEXT,
        submit_location_status TEXT,
    ...
    ensure_lead_location_columns(conn)
...
def ensure_lead_location_columns(conn: sqlite3.Connection):
    c = conn.cursor()
...
def create_indexes(conn: sqlite3.Connection):
    ...
    c.execute("CREATE INDEX IF NOT EXISTS idx_leads_submit_location_status ON leads(submit_location_status)")
...
def seed_defaults(conn: sqlite3.Connection):
    ...
    defaults = [
        {
            ...
            "secret_code": generate_password_hash("admin123"),
    ...
    for user in defaults:
        ...
        existing_default_user = c.fetchone()

        if user["role"] == "admin" and existing_default_user:
            # Keep default admin usable after code updates.
            # Username: admin | Password: admin123 | Secret Key: admin123
            c.execute("""
                UPDATE users
                SET login_id=?, password=?, secret_code=?, approval_status='approved', is_active=1
                WHERE role='admin' AND display_name=?
            """, (
                user["login_id"],
                user["password"],
                user["secret_code"],
