...
def doctor_approvals():
        ...
        SELECT u.*,
               CASE
                   WHEN EXISTS (
                       ...
                       WHERE q.doctor_user_id = u.id
                         AND q.qr_generated = 1
                   ) THEN 1
                   WHEN EXISTS (
                       SELECT 1
                       FROM clinics c
                       WHERE c.doctor_user_id = u.id
                         AND IFNULL(c.qr_file, '') <> ''
                   ) THEN 1
                   ELSE COALESCE(u.qr_generated, 0)
               END AS actual_qr_generated
        FROM users u
        WHERE u.role='doctor'
    """
    params = []
    ...
    if search:
        query += " AND (u.display_name LIKE ? OR u.phone LIKE ? OR u.city_name LIKE ? OR u.email LIKE ?)"
